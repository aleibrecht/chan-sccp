	...
	switch (ind) {
		case AST_CONTROL_RINGING:
			if (SKINNY_CALLTYPE_OUTBOUND == c->calltype) {
				// TODO: Replace by ast_rtp_instance_get_codecs(rtp->instance) instead
				{
					// Use ast_rtp_instance_get_codecs(c->rtp.adio.rtp) instead of code below
					...
					/* This is the first real change we have to fixup the capabilities of both sides */
					/* Reduce the number of allowed codecs */
...
//
// Implement allow_rtp_remote (so that we can ask if the other side can also do direct rtp, instead of trying blindly)
//
/*
...
static int sccp_wrapper_asterisk113_allow_rtp_remote(PBX_CHANNEL_TYPE *ast, struct ast_rtp_instance *instance)
{
        return check_rtp_remote_acl(ast, instance, SCCP_RTP_AUDIO);
}
#ifdef CS_SCCP_VIDEO[5~
