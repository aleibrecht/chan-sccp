...
void sccp_channel_openReceiveChannel(constChannelPtr channel)
{
	...
	if (!channel->rtp.audio.instance && !sccp_rtp_createServer(d, channel, SCCP_RTP_AUDIO)) {
...
int sccp_channel_receiveChannelOpen(sccp_device_t *d, sccp_channel_t *c)
{
	...
	if (c->owner && pbx_channel_state(c->owner) != AST_STATE_UP) {
		...
		if (c->calltype == SKINNY_CALLTYPE_OUTBOUND && SCCP_CHANNELSTATE_IsConnected(c->state)) {
			if (SCCP_RTP_STATUS_INACTIVE == c->rtp.audio.mediaTransmissionState && !d->directrtp) {
				sccp_channel_startMediaTransmission(c);
			}
